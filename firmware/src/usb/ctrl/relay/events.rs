...
async fn forward_binary(kind: u8, payload: &[u8]) -> bool {
    ...
    transfer::send_binary(out).await.is_ok()
}
...
pub(in crate::usb::ctrl) fn forward_filesystem_page(payload: &[u8]) -> bool {
    ...
    let mut out: Vec<u8, { transfer::TRANSFER_BINARY_MAX }> = Vec::new();
    if out.push(transfer::WS_BINARY_KIND_FILESYSTEM).is_err()
        || out.extend_from_slice(payload).is_err()
    {
        return false;
    }
...
pub(super) async fn emit_transfer_progress(state: &RelayTransferState) {
    let mut event: String<{ transfer::TRANSFER_TEXT_MAX }> = String::new();
    let _ = write!(
        &mut event,
    ...
    let _ = transfer::queue_text(event);
}
...
pub(super) async fn emit_transfer_finished(transfer_id: u64) {
    ...
    let _ = transfer::send_text(event).await;
}

...
fn app() -> Html {
    ...
    let push_log = {
        ...
        Callback::from(move |line: String| {
            ...
            v.push(line);
            if v.len() > 300 { let _ = v.drain(0..v.len()-300); }
            log_lines.set(v);
        })
    };
        ...
        use_effect_with((), move |_| {
            wasm_bindgen_futures::spawn_local(async move {
                ...
                match api::get_status().await {
                    Ok(s) => status.set(Some(StatusState { usb_enabled: s.usb_enabled, usb_ready: s.usb_ready, host_os: s.host_os })) ,
                    Err(e) => push_log.emit(format!("status error: {e}")),
                }
                match api::get_config().await {
                    Ok(c) => config.set(Some(ConfigState { usb_manufacturer: c.usb_manufacturer, usb_product: c.usb_product })),
            ...
            wasm_bindgen_futures::spawn_local(async move {
                ...
                match api::save_config(&man, &prod).await {
                    Ok(()) => {
                        ...
                        config.set(Some(ConfigState { usb_manufacturer: man, usb_product: prod }));
                    }
                    Err(e) => {
    ...
    let on_usb_start = {
        ...
        Callback::from(move |assistant: bool| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                set_busy.emit(true);
                let os_opt = if selected_os == "unknown" { None } else { Some(selected_os.as_str()) };
...
fn identity_card(props: &IdentityProps) -> Html {
    let man = use_state(|| props.config.as_ref().map(|c| c.usb_manufacturer.clone()).unwrap_or_default());
    let prod = use_state(|| props.config.as_ref().map(|c| c.usb_product.clone()).unwrap_or_default());
...
fn usb_card(props: &UsbProps) -> Html {
    ...
    let start_assist = { let cb = props.on_start.clone(); Callback::from(move |_| cb.emit(true)) };
    let start_noassist = { let cb = props.on_start.clone(); Callback::from(move |_| cb.emit(false)) };
    html! {
...
fn scripting_card(props: &ScriptProps) -> Html {
    let on_text = {
        ...
        Callback::from(move |e: InputEvent| cb.emit(e.target_unchecked_into::<web_sys::HtmlTextAreaElement>().value()))
    };

    let on_keydown = {
        let cb = props.on_run.clone();
        Callback::from(move |e: KeyboardEvent| {
            if (e.ctrl_key() || e.meta_key()) && e.key() == "Enter" { e.prevent_default(); cb.emit(()); }
        })
    };
...
#[derive(Properties, PartialEq, Clone)]
struct LogProps { pub lines: Vec<String>, pub on_clear: Callback<()> }
...
#[derive(Properties, PartialEq, Clone)]
struct ToastProps { pub toast: Option<(String, bool)> }

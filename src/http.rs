...
use picoserve::response::{self, Content, Response, StatusCode};
...
// Extractor for raw query string (avoids serde).
struct RawQuery<'a>(Option<picoserve::url_encoded::UrlEncodedString<'a>>);
impl<'r, State> picoserve::extract::FromRequestParts<'r, State> for RawQuery<'r> {
...
pub async fn server_task(stack: &'static net::Stack<'static>) -> ! {
    ...
    let app: Router<_> = Router::new()
        ...
        .route("/style.css", get(route_style))
        .route("/spider.svg", get(route_spider))
        .route(
            "/config",
        ...
        .route("/usb/register", post_service(UsbRegisterPostService));
...
async fn route_root() -> impl picoserve::response::IntoResponse {
...
async fn route_style() -> impl picoserve::response::IntoResponse {
...
async fn route_spider() -> impl picoserve::response::IntoResponse {
    static SVG: &str = include_str!("../assets/spider.svg");
    Response::ok(BytesWithType {

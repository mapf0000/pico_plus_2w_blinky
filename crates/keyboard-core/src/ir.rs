//! Language-neutral keyboard operations.
...
impl FlatProgram {
    ...
    pub fn push_tap(&mut self, usage: Usage, mods: Mods) -> Result<(), FlatProgramError> {
        self.ops.push(FlatOp::Tap { usage, mods });
        self.check_limit()
    }

    pub fn push_delay(&mut self, ms: u32) -> Result<(), FlatProgramError> {
        if ms == 0 {
            return Ok(());
        }

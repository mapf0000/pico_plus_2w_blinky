//! Keyboard layout selection and character mapping.
...
mod tests_win_de_de {
    use crate::{FlatOp, KEY_A, LayoutId, OpOwned, ProgramOwned, lower_to_flat_with_layout};
    ...
    fn layout_switches_text_lowering() {
        ...
        owned.ops.push(OpOwned::Layout(LayoutId::WinDeDe));
        owned.ops.push(OpOwned::Text {
            s: "y".into(),
            delay_ms: 0,
        });
...
mod tests_mac_de_de {
    use crate::{
        FlatOp, KEY_A, KEY_SPACE, LayoutId, MOD_LALT, Mods, OpOwned, ProgramOwned,
        lower_to_flat_with_layout,
    };

...
mod payloads {
    ...
    pub fn register_reruns(cfg: &Config) {
        let base = cfg.manifest_dir.join("builtin-scripts");
...
mod msc_image {
    ...
    pub fn register_reruns(_cfg: &Config) {
        cargo::rerun_if_changed("tools/host-agent/artifacts");
        cargo::rerun_if_changed("tools/host-agent/Cargo.toml");
    }
    ...
    pub fn prepare(cfg: &Config) -> Result<()> {
        ...
        let artifacts_dir = cfg.manifest_dir.join("tools/host-agent/artifacts");
    ...
    fn build_readme(status: &[TargetStatus], label: &str) -> String {
        ...
        let _ = writeln!(out, "tools/host-agent/artifacts/<target>/");
        out
    }
